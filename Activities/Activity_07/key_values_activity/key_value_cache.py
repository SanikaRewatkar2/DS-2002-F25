...
# user data here as another dictionary
USER_A_DATA = {
    "user_id": 9001,
...
# get data
import time #inbuilt module yayyayayayayay
...
data = session_cache.get(key_to_find)  # key value search! aren't dictionaries great?
...
found_slow = next((v for k, v in session_cache.items() if v['username'] == 'nosql_fan'), None) # forloop-esque search
...
new_activity.append("submitted_lab_4") # New activity! Add to recent_activity list

# Overwrite the old value with the new value
session_cache['user_session:9001']['recent_activity'] = new_activity # reassign it 2D-array style
